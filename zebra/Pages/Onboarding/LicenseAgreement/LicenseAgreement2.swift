...
struct LicenseAgreement2: View {
    @State var didAgree = false;
    var body: some View {
        VStack(){
            HStack(alignment: .center){
                Text("Политика конфиденциальности 2").font(.custom("Roboto-Regular", size: 20))
                    .fontWeight(.medium)
                    .padding()
            }
            ...
            HStack{
                Spacer()
                PushView(destination: LicenseAgreement3(), isActive: $didAgree){
...
struct LicenseAgreement2_Previews: PreviewProvider {
